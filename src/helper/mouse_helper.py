    ...
    def bernsteinPolynomial(points):
        ...
        def bern(t):
            ...
            for i, point in enumerate(points):
                _bern = BezierCurve.bernsteinPolynomialPoint(t, i, n)
                x += point[0] * _bern
                y += point[1] * _bern
            return x, y
...
class HumanCurve:
    ...
    def distortPoints(points, distortionMean, distortionStdev, distortionFrequency):
        ...
        for i in range(1, len(points) - 1):
            x, y = points[i]
            delta = (
                nprandom.normal(distortionMean, distortionStdev)
                if random() < distortionFrequency
                else 0
            )
            distorted += ((x, y + delta),)
...
def move_smooth(x, y, absolute=True, duration=0, randomize=4):
    """
