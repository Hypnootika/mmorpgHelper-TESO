...
import os
from src.helper import recorder_helper, image_helper, config_helper, name_helper
...
TOOLBOX_SUBSTRING = "Notepad Toolbox"
...
class Toolbox(QDialog):
    def __init__(self, parent=None):
        ...
        self.hwnd = self.toolbox_get_hwnd()
        self.setWindowIcon(QIcon("../assets/layout/mmorpg_helper.ico"))
        QApplication.setStyle(QStyleFactory.create("Fusion"))
        self.setWindowTitle(self.name + " Toolbox")
    ...
    def toolbox_get_hwnd(self, hwnd=None, classname=None):
        if hwnd is None:
            hwnd = FindWindow(classname, self.windowTitle())
            if hwnd == 0:
                raise Exception(self.name + " window not found", hwnd)
        else:
            hwnd = hwnd
        return hwnd
    ...
    def stop(self):
        ...
        if self.started:
            ...
            self.started = False
            self.parent.invisible_stopbtn.hide()
    ...
    def get_image_from_pos(self):
        ...
        if path.exists(self.image_path + self.image_name + ".png"):
            ...
            x, y = image_helper.get_image_at_cursor(
                self.image_name, self.image_path, self.x_coord, self.y_coord
            )
            info(
                "File saved as: %s location: %s position: x=%d, y=%d, size=%d, %d"
                % (
                    str(self.image_name + ".png"),
                    self.image_path,
                    x,
                    y,
                    self.x_coord,
